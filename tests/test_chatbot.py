...
class TestChatbot(TestCase):
  ...
  def test_has_all_methods(self):
    self.assertEqual(hasattr(self.chatbot, "add_intent"), True,
      "Chatbot: no add_intent method found")
    
    add_intent = self.chatbot.add_intent
    self.assertEqual(hasattr(add_intent, "__call__"), True,
      "Chatbot: add_intent is not a method")

    self.assertEqual(add_intent.__code__.co_argcount, 1+2,
  ...
  def test_correctly_adds_new_intent(self):
    ...
    self.assertEqual(self.chatbot.intents.get(intent_name), None,
      "Chatbot: intents should be empty when initialized")
    self.chatbot.add_intent(intent_name, intent_phrases)
    self.assertEqual(self.chatbot.intents.get(intent_name), intent_phrases,
